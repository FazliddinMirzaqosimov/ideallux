...
const CategoryCard = ({image, text, isCard,isActive}) => {
    ...
    return (
        <div
            ...
            <p className={`duration-700  font-bold xl:text-2xl lg:text-xl sm:text-lg text-sm px-6 font-GothamPro   ${isCard ? ' group-hover:text-white text-black absolute top-10 z-50' : `font-normal break-words  px-6 py-3 ${isActive ? 'text-white' : 'text-secondary'}`} `}>{text}  </p>
            {
                isCard &&
                <>
                    <div
                    ...
                    <div
                        className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 2xl:w-[85%] xl:w-[95%] w-[90%] 2xl:aspect-[3/5] lg:aspect-[4/5] aspect-[3/3] z-20`}>
